...
type TestRuntime<'m, 's> = Runtime<'m, 's>;
...
impl<'m, 's> Instances<'m, 's> {
    ...
    fn new_runtime(
        &self,
        m: &'m wain_ast::Module<'s>,
        pos: usize,
    ) -> Result<'s, TestRuntime<'m, 's>> {
        let runtime = Runtime::instantiate(m)
    ...
    fn invoke(&mut self, invoke: &wast::Invoke<'s>) -> Option<Bytes> {
        let (runtime, mod_pos) = self.find(invoke.id, invoke.start).unwrap();

        let args: Box<[Value]> = invoke.args.iter().map(|c| c.to_value().unwrap()).collect();
        //println!("name {:?}", &invoke.name);
        //println!("args {:?}", args);

        let ret = runtime.invoke(&invoke.name, &args).unwrap();
        match ret.clone() {
    ...
    fn test_command<'m>(
        ...
        match command {
            ...
            AssertTrap(wast::AssertTrap {
                ...
                validate(root)?;
                match Runtime::instantiate(&root.module) {
